  ...
  <div class="faqs" style="margin-bottom: 100px">
    ...
    <div class="mx-3 mt-7">
      <div class="questions" v-for="category in faqsList" :key="category.id">
        <h4 class="mb-2 font-weight-bold" :id="category.name" style="font-size: 18px">{{ category.name }}</h4>
        <v-divider></v-divider>

        <div class="mt-3" v-for="faqs in category.faqs" :key="faqs.question">
          <p>
            <a :href="'#' + faqs.question.replace(/\s/g, '-')" style="text-decoration: none; font-size: 14px">{{ faqs.question }}</a>
          </p>
        </div>
      ...
      <div class="answers mt-3" v-for="category in faqsList" :key="category.name">
        ...
        <div class="mt-5" v-for="faqs in category.faqs" :key="faqs.answer" :id="faqs.question.replace(/\s/g, '-')">
